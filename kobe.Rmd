...
```{r}
library("factoextra")
kobe.cont = kobe[,which(names(kobe) %in% c("season", "feet_x", "feet_y", "shot_distance", "shot_angle", "shot_type_num"))] # TODO shot_type_num!
pca = prcomp(kobe.cont[train_id,], scale=TRUE)
fviz_eig(pca)
res.var <- get_pca_var(pca)
res.var$coord
pca = prcomp(kobe.cont, scale=TRUE)
pca.cont = predict(pca, newdata=kobe.cont)[,1:3]
pca.kobe = data.frame(pca.cont, action_type, period, playoffs, opponent)
logreg = glm(shot_made_flag~pca.cont[,1]+pca.cont[,2]+pca.cont[,3]+action_type+period+playoffs+opponent, family=binomial, subset=train_id)
