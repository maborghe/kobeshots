...
### 4. Svm
In this section we will consider another popular classification technique, that is Support Vector Machines. The goal of this technique is to find a vector *w*, called *support vectors*, that satisfies the equation 
...
svmlin = svm(shot_made_bool~., data=dat, kernel="linear", cost=10, scale=TRUE)
...
library(e1071)
dat = kobe_frame[train_id,]
svmrad = svm(shot_made_bool~., data=dat, kernel="radial", gamma=1, cost=10, scale=FALSE)
preds = predict(svmrad, kobe_frame[-train_id,])
preds = ifelse(preds=="No", 0, 1)
sum(preds==test_y)/length(test_y)
```
